...
def load_data_generator(batch_size=32, mode='train'):
    input_path = f'/kaggle/input/two-cubes-data/Preprocessed_Data/{mode}/Input_images'
    ...
    target_path = f'/kaggle/input/two-cubes-data/Preprocessed_Data/{mode}/Target_images'
    ...
    action_path = f'/kaggle/input/two-cubes-data/Preprocessed_Data/{mode}/data_actions.npy'
...
def intro_vae_frame_prediction(device=torch.device("cuda:0"), batch_size=32,
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        if (epoch%20==0):
            torch.save(model.state_dict(), f'./{model_dir}/model_intro_vae_completed_after_{epoch}_epochs.pt')
